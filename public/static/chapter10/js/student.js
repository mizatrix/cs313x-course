...
function markVisited(vizId) {
  const visited = JSON.parse(localStorage.getItem('ch10_visited') || '[]');
  if (!visited.includes(vizId)) {
    visited.push(vizId);
    localStorage.setItem('ch10_visited', JSON.stringify(visited));
  }
}

function getVisited() {
  return JSON.parse(localStorage.getItem('ch10_visited') || '[]');
}
